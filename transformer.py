...
class DecoderLayer(keras.layers.Layer):
    ...
    def call(self, x, enc_output, look_ahead_mask, padding_mask, training=False):
        ...
        attn_enc_dec = self.attention(out1, enc_output, enc_output, padding_mask)
        attn_enc_dec = self.dropout2(attn_enc_dec, training=training)
        out2 = self.layernorm2(out1 + attn_enc_dec)
...
class Decoder(keras.Model):
    ...
    def call(self, enc_output, x, look_ahead_mask, padding_mask, training=False):
        x = self.embedding(x) + self.pos_embedding
        x = self.dropout(x, training=training)
...
def look_ahead_mask(size):
    return tf.cast(tf.linalg.band_part(tf.ones((size, size)), -1, 0), dtype=tf.float32)
...
class Transformer(keras.Model):
    ...
    def call(self, all_inputs):
        ...
        # FIX: training passed as keyword argument (not positional)
        enc_output = self.encoder(sources, enc_padding_mask, training=self.training)
        dec_output = self.decoder(enc_output, targets, combined_mask, dec_padding_mask, training=self.training)
